...
- `browser-harness --setup` attaches only to an existing authorized endpoint. It never opens or focuses Chrome or permission UI.
- `browser-harness --update -y` pulls code without recycling the live daemon. Never stop a healthy daemon during routine work.
- Never open `chrome://inspect`, request remote-debugging permission, or tell the user to click Allow. Authorization already exists. Failure means preserve browser state and return the attach diagnostic.
...
- **Attach-only is permanent.** Never open or focus Chrome, change browser settings, request permission, or recycle a healthy daemon.
- **Chrome Beta is first.** Local discovery reads its `DevToolsActivePort` before other profiles and skips stale ports.
- **All local agents share one daemon.** Reuse preserves the authorized connection and prevents repeated browser security prompts.
- **`DevToolsActivePort` can exist before its port listens.** The harness polls briefly, then fails with a diagnostic and no browser action.
- **No endpoint means no fallback UI.** Use an explicit `BU_CDP_WS` for isolated or remote work.
...
- **Stale connection means diagnose, not blind restart.** Recycling the daemon creates a new Chrome attach. Preserve it when healthy. Use `BU_CDP_WS` when isolation is required.
